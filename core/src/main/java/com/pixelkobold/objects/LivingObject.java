...
public class LivingObject extends GameObject {
    ...
    public void drawAnimations(SpriteBatch batch, float dt) {
		if (isMovingX || isMovingY) {
			stateTime += dt;
            switch (facing) {
                case DOWN -> batch.draw(downAnim.getKeyFrame(stateTime), pos.x, pos.y);
                case LEFT -> batch.draw(leftAnim.getKeyFrame(stateTime), pos.x, pos.y);
                case RIGHT -> batch.draw(rightAnim.getKeyFrame(stateTime), pos.x, pos.y);
                case UP -> batch.draw(upAnim.getKeyFrame(stateTime), pos.x, pos.y);
                default -> {
                }
            }
		} else {
            switch (facing) {
                case DOWN -> batch.draw(down, pos.x, pos.y);
                case LEFT -> batch.draw(left, pos.x, pos.y);
                case RIGHT -> batch.draw(right, pos.x, pos.y);
                case UP -> batch.draw(up, pos.x, pos.y);
                default -> {
                }
            }
		}
